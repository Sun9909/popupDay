...
public class FileDownloadController extends HttpServlet {
    ...
    public void fileDown(@RequestParam("articleNo") String articleNo, @RequestParam("imageFileName") String imageFileName,
        ...
        String path=ARTICLE_IMG_REPO + "\\" + articleNo + "\\" + imageFileName; // 이미지 경로, 이름
        File imageFile=new File(path);
        response.setHeader("Cache-Control","no-cache");

        // 이미지 파일 내려받는데(다운로드) 필요한 response 헤더 정보 설정
        response.addHeader("Content-disposition", "attachment;fileName=" + imageFileName); // attachement : 추가 fileName : 변수
